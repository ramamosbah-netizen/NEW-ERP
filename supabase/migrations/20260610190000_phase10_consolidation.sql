...
-- Relax role constraint on profiles
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_role_check;

-- RLS policies: Roles, Permissions, Mapping
DROP POLICY IF EXISTS "Allow authenticated read to roles" ON public.roles;
CREATE POLICY "Allow authenticated read to roles" ON public.roles FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow admin write to roles" ON public.roles;
...
DROP POLICY IF EXISTS "Allow authenticated read to permissions" ON public.permissions;
CREATE POLICY "Allow authenticated read to permissions" ON public.permissions FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow admin write to permissions" ON public.permissions;
...
DROP POLICY IF EXISTS "Allow authenticated read to role_permissions" ON public.role_permissions;
...
DROP POLICY IF EXISTS "Allow admin write to role_permissions" ON public.role_permissions;
...
DROP POLICY IF EXISTS "Allow authenticated read to settings" ON public.settings;
CREATE POLICY "Allow authenticated read to settings" ON public.settings FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Allow admin write to settings" ON public.settings;
...
DROP POLICY IF EXISTS "Allow audit read for admin and account" ON public.audit_log;
...
DROP POLICY IF EXISTS "Allow write to audit_log for logged in sessions" ON public.audit_log;
...
DROP POLICY IF EXISTS "Allow push subscriptions management by owner" ON public.push_subscriptions;
...
-- Auto-assign admin role to existing admin users
