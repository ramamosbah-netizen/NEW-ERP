...
# Quotation Tracker -> contract_value + full quote rows (for Phase-2 leads)
qval = {}; quotes = []
for r in rows('Quotation Tracker', 4):
    c = (r[0] or '').strip() if r and r[0] else ''
    if not re.match(r'^DSC-', c): continue
    v = num(r[10] if len(r) > 10 else None)
    if v != 'NULL': qval[c] = v
    quotes.append({
        'code': c, 'qno': re.sub(r'\s+', ' ', (r[1] or '').strip()) if len(r) > 1 else '',
        'client': norm_client(r[2] if len(r) > 2 else None) or 'TSC Limited (Sustainable City)',
        'desc': (str(r[3]).strip() if len(r) > 3 and r[3] else c),
        'system': systems(r[4] if len(r) > 4 else None),
        'issued': (r[5] if len(r) > 5 else None), 'valid': (r[6] if len(r) > 6 else None),
        'rev': (str(r[7]) if len(r) > 7 and r[7] else 'Rev.0'),
        'net': (v if v != 'NULL' else '0'),
        'status': (str(r[11]).strip() if len(r) > 11 and r[11] else ''),
    })
for p in projects:
...
A("BEGIN;")
A("")
A("-- admin user drives created_by (NOT NULL). Prefer the known admin id, then")
A("-- email, then the first user — whichever exists.")
A("SELECT set_config('jeet.admin_id',")
A("  COALESCE((SELECT id::text FROM auth.users WHERE id='da475069-0381-42da-b1b2-7af0cadd54d8'),")
A("           (SELECT id::text FROM auth.users WHERE email='ramma.mosbah@gmail.com' LIMIT 1),")
...
# ============================================================
# PHASE 2 — pre-award leads (Quotation Tracker rows not loaded as projects)
#           -> tenders + boqs + quotations + one summary line.
# ============================================================
OUT2 = os.path.join('supabase', 'migrations', '20260617000100_seed_leads.sql')
ADMIN = 'da475069-0381-42da-b1b2-7af0cadd54d8'
def tid(c): return str(uuid.uuid5(NS, 'tender:' + c))
def bid(c): return str(uuid.uuid5(NS, 'boq:' + c))
def qid(c): return str(uuid.uuid5(NS, 'quote:' + c))
def qlid(c): return str(uuid.uuid5(NS, 'qline:' + c))
def revno(r):
    m = re.search(r'(\d+)', str(r or '')); return int(m.group(1)) if m else 0
def tender_st(s):
    s = s.lower(); return 'Submitted' if ('submit' in s or 'negoti' in s or 'pending' in s) else 'Rejected'
def quote_st(s):
    s = s.lower()
    if 'submit' in s or 'negoti' in s or 'pending' in s: return 'SENT_TO_CLIENT'
    if 'lost' in s or 'cancel' in s: return 'REJECTED'
    return 'SUPERSEDED'  # expired / other

loaded = {p['code'] for p in projects}
leads = [qt for qt in quotes if qt['code'] not in loaded]

M = []; B = M.append
B("-- ============================================================")
B("-- JEET ERP — Phase 2: pre-award LEADS -> tenders + boqs + quotations")
B("-- GENERATED by scripts/seed_portfolio.py. Idempotent (ON CONFLICT).")
B("-- Apply AFTER 20260617000000_seed_portfolio.sql. One lead = tender->boq->quotation(+1 line).")
B("-- ============================================================")
B("BEGIN;")
B("SELECT set_config('jeet.admin_id',")
B(f"  COALESCE((SELECT id::text FROM auth.users WHERE id='{ADMIN}'),")
B("           (SELECT id::text FROM auth.users ORDER BY created_at LIMIT 1)), false);")
B("")
B("-- lead clients (upsert; most already exist from Phase 1)")
for name in sorted({qt['client'] for qt in leads}):
    B(f"INSERT INTO public.clients (id, name, country) VALUES ('{cid(name)}'::uuid, {q(name)}, 'UAE') "
      f"ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name;")
B("")
for qt in leads:
    c = qt['code']; net = numf(qt['net'])
    valid = d(qt['valid']); valid = valid if valid != 'NULL' else d(qt['issued'])
    valid_expr = f"COALESCE({valid}, CURRENT_DATE + 14)"
    B("INSERT INTO public.tenders (id, created_by, title, project_name, client_name, location, "
      "deadline_date, budget, status, scope_of_work) VALUES (")
    B(f"  '{tid(c)}'::uuid, current_setting('jeet.admin_id')::uuid, {q(qt['desc'])}, {q(qt['desc'])}, "
      f"{q(qt['client'])}, 'UAE', {valid_expr}, {net}, {q(tender_st(qt['status']))}, {q(qt['desc'])})")
    B("  ON CONFLICT (id) DO UPDATE SET title=EXCLUDED.title, budget=EXCLUDED.budget, status=EXCLUDED.status;")
    B("INSERT INTO public.boqs (id, tender_id, created_by, status, version, items, cost_elements, financials) VALUES (")
    B(f"  '{bid(c)}'::uuid, '{tid(c)}'::uuid, current_setting('jeet.admin_id')::uuid, 'draft', 1, "
      f"'[]'::jsonb, '{{}}'::jsonb, jsonb_build_object('total_selling_price', {net}, 'unit_selling_price', {net}))")
    B("  ON CONFLICT (id) DO UPDATE SET financials=EXCLUDED.financials;")
    B("INSERT INTO public.quotations (id, quotation_number, revision, status, boq_id, project_id, project_ref, "
      "client_id, client_name, quotation_date, valid_until, subject, subtotal_ex_vat, subtotal_after_discount, "
      "vat_amount, grand_total_with_vat, currency, prepared_by) VALUES (")
    B(f"  '{qid(c)}'::uuid, {q(qt['qno'] or ('Q-'+c))}, {revno(qt['rev'])}, {q(quote_st(qt['status']))}, "
      f"'{bid(c)}'::uuid, '{tid(c)}'::uuid, {q(c)}, '{cid(qt['client'])}'::uuid, {q(qt['client'])}, "
      f"COALESCE({d(qt['issued'])}, CURRENT_DATE), {valid_expr}, {q(qt['desc'])}, {net}, {net}, "
      f"round({net}*0.05,2), round({net}*1.05,2), 'AED', current_setting('jeet.admin_id')::uuid)")
    B("  ON CONFLICT (id) DO UPDATE SET status=EXCLUDED.status, subtotal_ex_vat=EXCLUDED.subtotal_ex_vat, "
      "grand_total_with_vat=EXCLUDED.grand_total_with_vat;")
    B("INSERT INTO public.quotation_lines (id, quotation_id, line_number, description, system, category, unit, "
      "quantity, unit_sell_price, unit_sell_price_after_discount, line_total, sort_order) VALUES (")
    B(f"  '{qlid(c)}'::uuid, '{qid(c)}'::uuid, 1, {q(qt['desc'])}, {q(qt['system'])}, {q(qt['system'])}, 'LS', "
      f"1, {net}, {net}, {net}, 1) ON CONFLICT (id) DO NOTHING;")
    B("")
B("NOTIFY pgrst, 'reload schema';")
B("COMMIT;")
B(f"-- summary: {len(leads)} leads -> tenders + boqs + quotations")
with open(OUT2, 'w', encoding='utf-8', newline='\n') as f:
    f.write('\n'.join(M))
print(f"leads={len(leads)} -> {OUT2}")
