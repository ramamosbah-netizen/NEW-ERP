...
A("-- Wipe-and-reseed in ONE transaction. Any FK error rolls the whole thing back (safe).")
A("-- Preserves users/auth/profiles/settings/workflows. Apply in the Supabase SQL editor.")
A("-- ============================================================")
A("BEGIN;")
A("")
...
A("")
A("-- 1) WIPE demo business data (child -> parent). Add tables here if an FK blocks.")
for t in ['client_invoice_items', 'payment_allocations', 'client_payments',
          'project_retention_ledger', 'client_invoices', 'projects',
          'pricing_suppliers', 'clients']:
    A(f"DELETE FROM public.{t};")
A("")
