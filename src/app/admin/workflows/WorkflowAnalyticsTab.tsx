...
// ============================================================
// JEET ERP — Workflow & Approvals Analytics (tab of /admin/workflows)
// Instances by status, pending approvals, SLA breaches and inventory.
// ============================================================
...
export default function WorkflowAnalyticsTab() {
  ...
  if (defs.length === 0 && insts.length === 0) return <Card><EmptyState icon={GitCompare} title="No workflows" description="Workflow definitions and their running instances will be analysed here." /></Card>;

  return (
    <div className="flex flex-col gap-5">
      <div className="flex justify-end gap-2">
        <Button variant="secondary" size="sm" icon={FileDown} onClick={() => exportTablePdf({ title: 'Workflow Definitions', columns: exportCols, rows: exportRows, fileName: 'workflow-definitions' })}>PDF</Button>
        <Button variant="secondary" size="sm" icon={Sheet} onClick={() => exportTableExcel({ title: 'Workflow Definitions', columns: exportCols, rows: exportRows, fileName: 'workflow-definitions' })}>Excel</Button>
      </div>
