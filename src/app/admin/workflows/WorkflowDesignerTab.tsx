...
// ============================================================
// JEET ERP — Workflow Designer (tab of /admin/workflows): definitions list
// ============================================================
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex justify-end">
        <Button variant="primary" size="sm" icon={Plus} onClick={() => setShowCreate(true)}>New workflow</Button>
      </div>
                ...
                {list.map(def => (
                  <div key={def.id} className="flex items-center justify-between gap-3 px-4 py-3 hover:bg-[var(--bg-card-hover)] transition-colors">
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <Button size="sm" variant="muted" icon={PencilLine} onClick={() => router.push(`/admin/workflows/${def.id}`)}>Design</Button>
                      <Button size="sm" variant="muted" icon={Copy} disabled={busy !== null} isLoading={busy === def.id} onClick={() => handleClone(def.id)}>Clone</Button>
            ...
            <form onSubmit={handleCreate} className="quote-modal-body flex flex-col gap-4">
              ...
              {newModule === '__CUSTOM__' && (
                <div className="quote-form-group">
                  ...
                  <input className="quote-form-input" placeholder="e.g. EQUIPMENT_CALIBRATION" value={customModule} onChange={e => setCustomModule(e.target.value)} />
                </div>
              )}
              <div className="quote-form-group">
