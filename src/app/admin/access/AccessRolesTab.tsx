...
// ============================================================
// JEET ERP — Access & Roles (tab of /admin/access)
...
export default function AccessRolesTab() {
  ...
  if (roles.length === 0) return <Card><EmptyState icon={Users} title="No roles" description="Roles and their access will be analysed here once the role catalogue is configured." /></Card>;

  return (
    <div className="flex flex-col gap-5">
      <div className="flex justify-end gap-2">
        <Button variant="secondary" size="sm" icon={FileDown} onClick={() => exportTablePdf({ title: 'Roles & Access', columns: exportCols, rows: exportRows, fileName: 'roles-access' })}>PDF</Button>
        <Button variant="secondary" size="sm" icon={Sheet} onClick={() => exportTableExcel({ title: 'Roles & Access', columns: exportCols, rows: exportRows, fileName: 'roles-access' })}>Excel</Button>
      </div>
