...
// ============================================================
// JEET ERP — Permissions Matrix (tab of /admin/access)
...
export default function PermissionsTab() {
  ...
  if (roles.length === 0 || perms.length === 0) return <Card><EmptyState icon={KeySquare} title="No permission model" description="The roles × permissions matrix will appear here once roles and permissions are configured." /></Card>;

  return (
    <div className="flex flex-col gap-5">
      <div className="flex justify-end gap-2">
        <Button variant="secondary" size="sm" icon={FileDown} onClick={() => exportTablePdf({ title: 'Permissions Matrix', subtitle: 'Granted / total permissions per role and module', columns: exportCols, rows: exportRows, fileName: 'permissions-matrix' })}>PDF</Button>
        <Button variant="secondary" size="sm" icon={Sheet} onClick={() => exportTableExcel({ title: 'Permissions Matrix', columns: exportCols, rows: exportRows, fileName: 'permissions-matrix' })}>Excel</Button>
      </div>
