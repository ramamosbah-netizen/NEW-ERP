...
// ============================================================
// JEET ERP — Platform Configuration Audit (tab of /admin/audit)
// Inventory of every no-code config object (workflows, forms,
// templates, rules) with active/inactive, version and module coverage.
// ============================================================
...
export default function ConfigurationTab() {
  ...
  if (items.length === 0) return <Card><EmptyState icon={SlidersHorizontal} title="No configuration objects" description="Workflows, forms, templates and rules will be inventoried here once configured." /></Card>;

  return (
    <div className="flex flex-col gap-5">
      <div className="flex justify-end gap-2">
        <Button variant="secondary" size="sm" icon={FileDown} onClick={() => exportTablePdf({ title: 'Configuration Inventory', columns: exportCols, rows: exportRows, fileName: 'configuration-audit' })}>PDF</Button>
        <Button variant="secondary" size="sm" icon={Sheet} onClick={() => exportTableExcel({ title: 'Configuration Inventory', columns: exportCols, rows: exportRows, fileName: 'configuration-audit' })}>Excel</Button>
      </div>
