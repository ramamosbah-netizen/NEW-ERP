...
// ============================================================
// JEET ERP — Audit & Activity Analytics
// Trends over the audit_log: activity by module / action / actor /
// time, with a security lens on DELETE / APPROVE / SYSTEM changes.
// Read-only; batched separate-lookup for actor names. PDF + Excel.
// ============================================================
...
export default function AuditAnalyticsPage() {
  ...
  return (
    <div className="flex flex-col gap-5">
      <PageHeader
        title="Audit & Activity Analytics"
        subtitle="Platform activity by module, action and actor — with a security lens"
        breadcrumbs={[{ label: 'Administration', href: '/admin/hub' }, { label: 'Audit', href: '/admin/audit' }, { label: 'Analytics' }]}
        actions={events.length > 0 ? (
          <div className="flex gap-2">
            <Button variant="secondary" icon={FileDown} onClick={() => exportTablePdf({ title: 'Audit Log', columns: exportCols, rows: exportRows, fileName: 'audit-log' })}>PDF</Button>
            <Button variant="secondary" icon={Sheet} onClick={() => exportTableExcel({ title: 'Audit Log', columns: exportCols, rows: exportRows, fileName: 'audit-log' })}>Excel</Button>
          </div>
        ) : undefined}
      />
      ...
      {loading ? (
        ...
        <Card><EmptyState icon={Activity} title="No audit events" description="Platform activity will be analysed here as users create, update and approve records." /></Card>
      ) : (
        <>
