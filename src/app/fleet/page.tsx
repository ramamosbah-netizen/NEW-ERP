...
export default function FleetDashboardPage() {
  ...
  const getDaysRemainingStr = (dateStr: string) => {
    ...
    if (diffDays < 0) return { text: `Expired by ${Math.abs(diffDays)} days`, style: 'bg-red-500/15 text-red-400 border border-red-500/35' };
    if (diffDays === 0) return { text: 'Expires Today', style: 'bg-red-500/15 text-red-400 border border-red-500/35 animate-pulse' };
    if (diffDays <= 7) return { text: `${diffDays} days left`, style: 'bg-red-500/15 text-red-400 border border-red-500/30' };
    if (diffDays <= 30) return { text: `${diffDays} days left`, style: 'bg-amber-500/15 text-amber-400 border border-amber-500/30' };
    return { text: `${diffDays} days left`, style: 'bg-slate-900 text-slate-400 border border-slate-800' };
  };
...
<main className="max-w-[1600px] mx-auto px-6 py-8">
        ...
        {criticalAlertsCount > 0 && (
          <div className="mb-8 p-4 bg-red-950/45 border border-red-500/30 rounded-xl flex items-center justify-between shadow-[0_0_20px_rgba(239,68,68,0.08)]">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 bg-red-500/10 border border-red-500/25 rounded-lg flex items-center justify-center text-red-400 animate-pulse">
                <ShieldAlert size={20} />
              </div>
              <div>
            ...
            <Link 
              ...
              className="text-xs font-bold text-slate-100 bg-red-500/20 border border-red-500/30 px-3 py-1.5 rounded-lg hover:bg-red-500/30 transition-all font-mono uppercase tracking-wider"
              onClick={() => {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            <Link 
              href="/fleet/fines"
              className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold bg-slate-900 border border-slate-800 text-slate-200 rounded-lg hover:bg-slate-800 transition-all"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="p-5 bg-[var(--bg-card)] backdrop-blur-xl border border-[var(--border-color)] rounded-xl relative overflow-hidden group">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-heading font-extrabold text-slate-100">{totalVehicles}</span>
              <span className="text-xs text-slate-500">units</span>
            </div>
            <div className="flex items-center gap-3 mt-4 text-[10px] font-mono text-slate-400 border-t border-slate-900 pt-3">
          ...
          <div className="p-5 bg-[var(--bg-card)] backdrop-blur-xl border border-[var(--border-color)] rounded-xl relative overflow-hidden group">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-heading font-extrabold text-slate-100">
                {formatAED(totalMTDCosts)}
              </span>
            </div>
            <div className="grid grid-cols-3 gap-2 mt-4 text-[9px] font-mono text-slate-400 border-t border-slate-900 pt-3">
              <div>
                <span className="block text-slate-500">Fuel</span>
                <span className="font-bold text-slate-300">{formatAED(fuelCostMTD)}</span>
              </div>
              <div>
                <span className="block text-slate-500">Service</span>
                <span className="font-bold text-slate-300">{formatAED(maintCostMTD)}</span>
              </div>
              <div>
                <span className="block text-slate-500">Fines</span>
                <span className="font-bold text-slate-300">{formatAED(finesCostMTD)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 bg-[var(--bg-card)] backdrop-blur-xl border border-[var(--border-color)] rounded-xl relative overflow-hidden group">
            ...
            <div className="flex justify-between items-center mt-4 text-[10px] font-mono text-slate-400 border-t border-slate-900 pt-3">
              <span>Unpaid Violations:</span>
              <span className="text-red-400 font-bold">{outstandingFinesCount} fines</span>
            </div>
          </div>
          ...
          <div className="p-5 bg-[var(--bg-card)] backdrop-blur-xl border border-[var(--border-color)] rounded-xl relative overflow-hidden group">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-heading font-extrabold text-slate-100">{expiriesList.length}</span>
              <span className="text-xs text-slate-500">vehicles</span>
            </div>
            <div className="flex justify-between items-center mt-4 text-[10px] font-mono text-slate-400 border-t border-slate-900 pt-3">
              <span>Critical Overdue:</span>
              <span className={`font-bold ${criticalAlertsCount > 0 ? 'text-red-400' : 'text-slate-400'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="lg:col-span-2 p-5 bg-[var(--bg-card)] backdrop-blur-xl border border-[var(--border-color)] rounded-xl flex flex-col h-[380px]">
            <h3 className="font-heading font-bold text-xs uppercase tracking-widest text-slate-300 mb-4 flex items-center gap-1.5">
              ...
              {expiriesList.length === 0 ? (
                ...
                expiriesList.map(v => {
                  ...
                  return (
                    <div 
                      key={v.id}
                      className="p-3 bg-slate-950/50 border border-slate-900 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:border-slate-800 transition-all"
                    >
                      <div>
                      ...
                      <div className="flex items-center gap-4 text-xs">
                        <div className="flex flex-col sm:items-end">
                          <span className="text-[10px] text-slate-500 uppercase tracking-widest block">Mulkiya (Reg)</span>
                          <span className={`mt-1 text-[10px] font-mono font-bold px-2 py-0.5 rounded ${regDays.style}`}>
                            {regDays.text}
                          </span>
                        </div>
                        <div className="flex flex-col sm:items-end">
                          <span className="text-[10px] text-slate-500 uppercase tracking-widest block">Insurance</span>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="p-5 bg-[var(--bg-card)] backdrop-blur-xl border border-[var(--border-color)] rounded-xl flex flex-col h-[180px]">
              <h3 className="font-heading font-bold text-xs uppercase tracking-widest text-slate-300 mb-3 flex items-center gap-1.5">
                ...
                {blackPointsWatchlist.length === 0 ? (
                  ...
                  <div className="gap-2 flex flex-col">
                    {blackPointsWatchlist.map((d: any) => (
                      <div key={d.name} className="flex justify-between items-center text-xs p-2 bg-slate-950/40 border border-slate-900 rounded-lg">
                        <div>
                          <div className="font-semibold text-slate-200">{d.name}</div>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] font-mono text-slate-400">Total:</span>
                          <span className={`font-bold font-mono text-sm ${d.points >= 22 ? 'text-red-500 animate-pulse' : d.points >= 18 ? 'text-red-400' : 'text-amber-400'}`}>
                            {d.points} / 24
                          </span>
                        </div>
                      </div>
                ...
                {upcomingServices.length === 0 ? (
                  ...
                  <div className="gap-2 flex flex-col">
                    {upcomingServices.map(m => (
                      <div key={m.id} className="flex justify-between items-center text-xs p-2 bg-slate-950/40 border border-slate-900 rounded-lg">
                        <div>
                          <div className="font-bold text-slate-300">Vehicle: {m.vehicle_code}</div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-950/40 text-xs">
                  {filteredVehicles.map(v => (
                    <tr key={v.id} className="hover:bg-slate-900/10 group">
                      <td className="py-4 font-bold font-mono text-[var(--primary)]">
                        {v.vehicle_code}
                      </td>
                      <td className="py-4">
                        <span className="font-mono bg-slate-950/80 px-2 py-0.5 rounded border border-slate-900 text-slate-300">
                          {v.plate_number}
                        </span>
                      </td>
                      ...
                      <td className="py-4">
                        <span className="font-semibold text-slate-200">{v.make} {v.model}</span>
                        <span className="text-[10px] text-slate-500 ml-1">({v.year})</span>
                      </td>
