...
export default function Dashboard() {
  ...
  const fetchSessionAndData = async () => {
    try {
      ...
      // 2. Fetch corresponding profile row
  ...
  const handleAckAlert = async (id: string) => {
    try {
      ...
      // Reload KPI stats to update warning count
      const kpis = await kpiService.getExecutiveKPIs();
      setKpiData(kpis);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#060814] flex flex-col items-center justify-center text-center p-6">
        ...
        <p className="text-slate-400 text-xs mt-1">Aggregating corporate metrics cockpit...</p>
      </div>
    );
  }
  ...
  // Prepare Pie Chart data for projects status distribution
  ...
  return (
    <div className="min-h-screen bg-[#060814] text-slate-100 flex flex-col font-sans">
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 py-8 flex flex-col gap-6">
        ...
        {/* Warning banner if sql setup failed */}
        {isFallback && (
          <div className="flex gap-3 bg-amber-500/10 border border-amber-500/20 rounded-lg p-4 text-amber-200 text-xs items-start">
            <AlertTriangle className="flex-shrink-0 text-amber-400" size={18} />
            <div>
              <strong>Profile Sync Mode Active</strong>
              <p className="mt-1">
                Unable to locate a matching record in the <code>public.profiles</code> table. Falling back to session cache.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-2 flex flex-col gap-4 min-w-0">
            <div>
        ...
        {/* Expiry alerts and quick navigations grid */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">

          {/* Document & Compliance Warning Feed */}
          <Card className="flex flex-col gap-3 min-h-[300px]">
            <div className="flex justify-between items-center border-b border-white/5 pb-2">
              ...
              <span className="bg-amber-400/10 text-amber-400 border border-amber-400/20 text-[10px] px-2 py-0.5 rounded font-mono font-bold">
          ...
          {/* Quick Module Navigation and Shortcuts */}
              ...
              {[
                ...
                return (
                  <Link key={item.href} href={item.href} className="no-underline group">
                    <div className="bg-[#0b0f2a] border border-white/5 hover:border-emerald-400/30 p-3 rounded flex flex-col gap-2 h-full transition-all cursor-pointer">
