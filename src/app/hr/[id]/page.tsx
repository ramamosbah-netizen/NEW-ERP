  ...
  if (error || !employee) {
    return (
      <div className="quote-container">
        <div className="bg-red-500/10 border border-red-500/30 text-red-400 p-6 rounded font-mono text-xs max-w-lg mx-auto mt-12">
    ...
    <div className="quote-container">
      ...
      <div className="quote-card mb-6" style={{ background: 'linear-gradient(135deg, rgba(13, 17, 39, 0.75) 0%, rgba(6, 8, 20, 0.95) 100%)', borderLeft: '3px solid var(--primary)' }}>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-slate-900 border border-slate-800 rounded-full flex items-center justify-center text-slate-500 font-bold text-lg">
              {employee.full_name_en.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-lg font-bold text-slate-100">{employee.full_name_en}</h1>
                <span className="text-xs text-slate-400 font-mono" dir="rtl">{employee.full_name_ar}</span>
              </div>
              <p className="text-xs text-slate-400 font-semibold">{employee.designation} — <span className="text-[10px] font-mono text-emerald-400 uppercase">{employee.department}</span></p>
              <div className="flex gap-4 text-[10px] text-slate-500 font-mono mt-1">
                <span>EMP NO: <strong className="text-slate-300">{employee.employee_number}</strong></span>
                <span>JOINED: <strong className="text-slate-300">{new Date(employee.join_date).toLocaleDateString('en-GB')}</strong></span>
      ...
      <div className="quote-card">
        ...
        {activeTab === 'profile' && (
          <div className="space-y-6">
            <h3 className="text-xs font-mono text-emerald-400 uppercase tracking-widest font-bold">Employee Record Profile</h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-xs font-mono">
              <div>
                ...
                <span className="text-slate-200 mt-1 block">{employee.nationality}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Date of Birth</span>
                <span className="text-slate-200 mt-1 block">{new Date(employee.dob).toLocaleDateString('en-GB')}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Gender</span>
                <span className="text-slate-200 mt-1 block">{employee.gender}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Mobile Connection</span>
                <span className="text-slate-200 mt-1 block">{employee.mobile}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Personal Email</span>
                <span className="text-slate-200 mt-1 block">{employee.personal_email}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Employment Type</span>
                <span className="text-slate-200 mt-1 block">{employee.employment_type.replace('_', ' ')}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Visa Sponsor</span>
                <span className="text-slate-200 mt-1 block">{employee.visa_sponsor === 'JEET' ? 'JEET Security' : 'Other'}</span>
              </div>
              <div>
                <span className="text-slate-500 block">MOHRE Person Code</span>
                <span className="text-slate-200 mt-1 block">{employee.mohre_person_code}</span>
              </div>
            </div>

            <div className="border-t border-slate-900 pt-6">
              <h4 className="text-[10px] font-mono text-slate-500 uppercase tracking-widest mb-4">WPS Salary Transfer Parameters</h4>
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-6 text-xs font-mono">
                <div>
                  <span className="text-slate-500 block">Bank Name</span>
                  <span className="text-slate-200 mt-1 block">{employee.bank_name}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">IBAN</span>
                  <span className="text-slate-200 mt-1 block text-cyan-400">{employee.iban}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">Routing Code</span>
                  <span className="text-slate-200 mt-1 block">{employee.routing_code}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">WPS Agent ID</span>
                  <span className="text-slate-200 mt-1 block">{employee.agent_id}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'documents' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-xs font-mono text-emerald-400 uppercase tracking-widest font-bold">Compliance Document Ledger</h3>
                ...
                {documents.map((docLink) => (
                  <div key={docLink.id} className="hr-glass-card p-4 flex justify-between items-start gap-4">
                    <div>
                      ...
                      <h4 className="font-semibold text-xs text-slate-100 mt-2">
                    ...
                    <button 
                      ...
                      className="text-slate-500 hover:text-rose-400 p-1 rounded hover:bg-slate-900 border border-slate-900 hover:border-rose-500/25 transition-all"
        ...
        {activeTab === 'certifications' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-xs font-mono text-emerald-400 uppercase tracking-widest font-bold">Employee Certifications Registry</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {certifications.map((cert) => (
                  <div key={cert.id} className="hr-glass-card p-4 flex justify-between items-start gap-4">
                    <div>
                      <div className="flex items-center gap-2">
                        <FileBadge className="text-cyan-400" size={16} />
                        <h4 className="font-semibold text-xs text-slate-100">
                          {cert.cert_type.replace('_', ' ')}
                        </h4>
                      </div>
                      <p className="text-[10px] font-mono text-slate-400 mt-2">No: <strong className="text-slate-200">{cert.cert_number}</strong></p>
                      <div className="grid grid-cols-2 gap-4 mt-2 text-[9px] font-mono text-slate-500">
                        <span>Issued: {new Date(cert.issue_date).toLocaleDateString('en-GB')}</span>
                        <span>Expires: <strong className={new Date(cert.expiry_date) < new Date() ? 'text-rose-400' : 'text-slate-300'}>{new Date(cert.expiry_date).toLocaleDateString('en-GB')}</strong></span>
                      </div>
                    </div>
                    <button 
        ...
        {activeTab === 'compensation' && hasCompAccess && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-xs font-mono text-emerald-400 uppercase tracking-widest font-bold">Confidential Salary Compensation History</h3>
                  ...
                  <tbody>
                    {compensations.map((comp) => (
                      <tr key={comp.id}>
                        ...
                        <td className="text-[10px] text-slate-400">{comp.notes || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {showLinkDocModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 rounded-lg max-w-md w-full p-6">
            <h3 className="text-xs font-mono text-emerald-400 uppercase tracking-widest font-bold mb-4">Link Document from DMS</h3>
            <form onSubmit={handleLinkDoc} className="space-y-4">
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">DMS File Reference *</label>
                <select 
                  required 
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Document Classification *</label>
                <select 
                  required 
                  className="quote-filter-input w-full" 
      ...
      {showCertModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 rounded-lg max-w-md w-full p-6">
            <h3 className="text-xs font-mono text-emerald-400 uppercase tracking-widest font-bold mb-4">Add Certification Card</h3>
            <form onSubmit={handleAddCert} className="space-y-4">
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Certification Type *</label>
                <select 
                  required 
                  className="quote-filter-input w-full" 
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Certificate Number *</label>
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Issue Date *</label>
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Expiry Date *</label>
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Link DMS File (Optional)</label>
                <select 
                  className="quote-filter-input w-full" 
      ...
      {showCompModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 rounded-lg max-w-md w-full p-6">
            <h3 className="text-xs font-mono text-emerald-400 uppercase tracking-widest font-bold mb-4">Update Salary Parameters</h3>
            <form onSubmit={handleAddComp} className="space-y-4">
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Effective Date *</label>
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Basic Salary (AED) *</label>
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Housing Allowance (AED)</label>
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Transport Allowance (AED)</label>
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Other Allowances (AED)</label>
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Burden Multiplier *</label>
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase mb-1">Notes / Rationale</label>
