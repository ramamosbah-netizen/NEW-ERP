...
<main className="flex-1 max-w-7xl w-full mx-auto px-6 py-8 flex flex-col gap-6">
        ...
        <div className="flex justify-between items-start gap-4 flex-wrap">
          <div>
            <div className="text-[10px] text-slate-500 font-mono uppercase tracking-widest flex items-center gap-1">
              <Link href="/finance" className="hover:text-emerald-400 flex items-center gap-0.5"><ArrowLeft size={10} /> Finance Command</Link> &gt; <span className="text-slate-300">Cash Flow Forecast</span>
            </div>
          ...
          <div className="flex items-center gap-3">
            <form onSubmit={handleUpdateOpeningBalance} className="flex items-center gap-2 bg-slate-950 border border-slate-900 rounded p-1">
              <span className="text-[9px] font-mono text-slate-500 uppercase px-2">Opening Cash Balance:</span>
              <input
                type="number"
                value={openingInput}
                onChange={(e) => setOpeningInput(Number(e.target.value))}
                className="bg-[#0a0f26] border border-slate-900 rounded px-2.5 py-1 text-xs text-slate-100 font-mono w-32 focus:outline-none focus:border-emerald-500"
              />
              <button
                type="submit"
                className="bg-emerald-500 text-slate-950 text-[10px] font-mono uppercase font-bold px-3 py-1 rounded transition-all hover:bg-emerald-400 cursor-pointer"
              >
                Set
              </button>
            </form>
            ...
            <button
              onClick={() => refetch()}
              className="bg-slate-900 border border-slate-800 hover:border-slate-700 p-2 rounded text-slate-400 hover:text-slate-200 transition-all cursor-pointer"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-950/60 border border-slate-900 rounded p-4">
            <span className="text-slate-500 block uppercase font-mono text-[9px] mb-1">Peak Cash Position</span>
            <span className="font-mono text-base font-extrabold text-emerald-400 block">{formatAED(peakCash)}</span>
            <span className="text-[10px] text-slate-400 block mt-0.5">Maximum projected surplus</span>
          </div>

          <div className="bg-slate-950/60 border border-slate-900 rounded p-4">
            <span className="text-slate-500 block uppercase font-mono text-[9px] mb-1">Min Cash Position</span>
            <span className="font-mono text-base font-extrabold text-amber-500 block">{formatAED(minCash)}</span>
            <span className="text-[10px] text-slate-400 block mt-0.5">Maximum projected drawdown</span>
          </div>

          <div className="bg-slate-950/60 border border-slate-900 rounded p-4">
            <span className="text-slate-500 block uppercase font-mono text-[9px] mb-1">Net Forecasted Flow</span>
            <span className={`font-mono text-base font-extrabold block ${netForecastedFlow >= 0 ? 'text-emerald-400' : 'text-rose-500'}`}>
              {netForecastedFlow >= 0 ? '+' : ''}{formatAED(netForecastedFlow)}
            </span>
            <span className="text-[10px] text-slate-400 block mt-0.5">13-week cumulative net</span>
          </div>

          <div className="bg-slate-950/60 border border-slate-900 rounded p-4">
            <span className="text-slate-500 block uppercase font-mono text-[9px] mb-1">Projected Ending Cash</span>
            <span className="font-mono text-base font-extrabold text-slate-200 block">{formatAED(closingBalance)}</span>
            <span className="text-[10px] text-slate-400 block mt-0.5">Estimated cash on hand in week 13</span>
          </div>
        </div>
            ...
            <table className="w-full border-collapse text-left text-xs">
              <thead>
                <tr className="bg-slate-950 border-b border-slate-900 text-slate-400 font-mono uppercase tracking-wider text-[10px]">
                  ...
                  <th className="py-3.5 px-4 text-right text-emerald-400">Total Inflow</th>
                  <th className="py-3.5 px-4 text-right">Outflow (AP)</th>
                  <th className="py-3.5 px-4 text-right">Outflow (POs)</th>
                  <th className="py-3.5 px-4 text-right text-rose-400">Total Outflow</th>
                ...
                {loading ? (
                  ...
                  forecast.map((week) => {
                    ...
                    return (
                      <tr 
                        key={week.weekIndex} 
                        className="border-b border-slate-900/50 hover:bg-[#0a0f26]/40 transition-all font-mono"
                      >
                        ...
                        <td className="py-3 px-4 text-right text-emerald-400/90 font-bold">
                        ...
                        <td className="py-3 px-4 text-right text-rose-400/90 font-bold">
                          {formatAED(week.outflows.total)}
                        </td>
                        <td className={`py-3 px-4 text-right font-bold ${isPositive ? 'text-emerald-400' : 'text-rose-500'}`}>
