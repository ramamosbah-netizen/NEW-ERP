...
<main className="flex-1 max-w-5xl w-full mx-auto px-6 py-8 flex flex-col gap-6">
        ...
        <div>
          <div className="text-[10px] text-slate-500 font-mono uppercase tracking-widest flex items-center gap-1">
            <Link href="/finance/ar" className="hover:text-emerald-400 flex items-center gap-0.5"><ArrowLeft size={10} /> Registry</Link> &gt; <span>New Draft</span>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 bg-slate-950/40 border border-slate-900 rounded p-6 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] text-slate-400 font-mono uppercase mb-1.5">Project Reference</label>
                <select
                  value={projectId}
              ...
              <div>
                <label className="block text-[10px] text-slate-400 font-mono uppercase mb-1.5">Client Profile</label>
                <select
                  value={clientId}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                <label className="block text-[10px] text-slate-400 font-mono uppercase mb-1.5">Invoice Type</label>
                <select
                  value={invoiceType}
              ...
              <div>
                <label className="block text-[10px] text-slate-400 font-mono uppercase mb-1.5">Supply Date</label>
                <input
                  type="date"
                  value={supplyDate}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] text-slate-400 font-mono uppercase mb-1.5">Billing Period (From)</label>
                <input
                  type="date"
                  value={periodFrom}
              ...
              <div>
                <label className="block text-[10px] text-slate-400 font-mono uppercase mb-1.5">Billing Period (To)</label>
                <input
                  type="date"
                  value={periodTo}
            ...
            {invoiceType === 'PROGRESS' && (
              <div className="bg-[#0a0f26] border border-slate-900 rounded p-4 flex flex-col gap-4">
                <h4 className="text-[10px] font-mono text-emerald-400 uppercase tracking-widest">
                  Progress Claim Adjustments
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[9px] text-slate-500 font-mono uppercase mb-1">Advance Recovery Rate (%)</label>
                    <input
                      type="number"
                      step="1"
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded py-2 px-3 text-xs text-slate-300 focus:outline-none focus:border-emerald-500/50 font-mono"
                    />
                  </div>
                  <div>
                    <label className="block text-[9px] text-slate-500 font-mono uppercase mb-1">Retention Held Rate (%)</label>
                    <input
                      type="number"
                      step="1"
                  ...
                  <div>
                    <label className="block text-[9px] text-slate-500 font-mono uppercase mb-1">Certified Gross Claim (AED)</label>
                    <input
                      type="number"
                      step="0.01"
            ...
            <div className="flex flex-col gap-3 mt-2">
              <div className="flex justify-between items-center border-b border-slate-900 pb-2">
                <span className="text-[10px] text-slate-400 font-mono uppercase">Itemized invoice lines</span>
                <button
                  type="button"
                  onClick={addItemRow}
              ...
              {items.map((item, idx) => (
                <div key={idx} className="flex flex-col md:flex-row gap-3 items-start md:items-center bg-[#0a0f26]/30 p-3 rounded border border-slate-900/60">
                  ...
                  <div className="w-20">
                    <input
                      ...
                      className="w-full bg-[#0a0f26] border border-slate-800 rounded py-1.5 px-3 text-xs text-slate-300 focus:outline-none focus:border-emerald-500/50 font-mono text-right"
                      required
                    />
                  </div>
                  <div className="w-16">
                    <input
                      ...
                      className="w-full bg-[#0a0f26] border border-slate-800 rounded py-1.5 px-3 text-xs text-slate-300 focus:outline-none focus:border-emerald-500/50 text-center"
                      required
                    />
                  </div>
                  <div className="w-28">
                  ...
                  <div className="w-28 text-right font-mono font-bold text-slate-300 pr-2 self-center">
                  ...
                  {items.length > 1 && (
                    <button
                      ...
                      className="p-1.5 text-slate-600 hover:text-red-400 transition-all self-center"
                    >
                      <Trash2 size={14} />
                    </button>
                  )}
            ...
            <div>
              <label className="block text-[10px] text-slate-400 font-mono uppercase mb-1.5">Invoice Notes</label>
              <textarea
                rows={3}
          ...
          <div className="w-full lg:w-80 bg-slate-950/60 border border-slate-900 rounded p-6 flex flex-col gap-6 h-fit">
            <h3 className="text-xs font-mono text-emerald-400 uppercase tracking-wider border-b border-slate-900 pb-2">
            ...
            {selectedProj && (
              <div className="bg-[#0b122c] border border-[#162754] rounded p-3 text-xs flex flex-col gap-1.5">
                <div className="text-[10px] text-slate-400 font-mono uppercase tracking-wider">Project Contract Status</div>
                ...
                {selectedProj.revised_contract_value !== undefined && Number(selectedProj.revised_contract_value) !== Number(selectedProj.original_contract_value ?? selectedProj.contract_value) && (
                  <>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Approved VOs:</span>
                    ...
                    <div className="flex justify-between border-t border-slate-800 pt-1.5 font-bold">
                      <span className="text-slate-200">Revised Limit:</span>
                      <span className="font-mono text-emerald-400">
            ...
            <div className="flex flex-col gap-3.5 text-xs">
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono text-slate-200">{totals.taxable_amount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400">VAT (5%):</span>
                <span className="font-mono text-slate-200">{totals.vat_amount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center border-b border-slate-900 pb-3">
                <span className="text-slate-400 font-semibold">Total (Incl. VAT):</span>
                <span className="font-mono font-bold text-slate-100">{totals.total_incl_vat.toFixed(2)}</span>
              </div>
              ...
              <div className="flex justify-between items-center bg-[#0a0f26] p-3 rounded border border-slate-900 mt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-emerald-400 text-slate-950 font-bold rounded hover:bg-emerald-300 transition-all uppercase tracking-wider text-xs shadow-[0_0_15px_rgba(0,229,160,0.15)] disabled:opacity-50"
            >
