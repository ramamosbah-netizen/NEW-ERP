...
- ✅ **Phase 11 — Training & Competency Matrix** (`/hr/competency`): employee × skill grid with inline 0–4 level editing (colour-graded), add-skill, plus a training-records tab (add/remove). New `competencyService` (audit-logged, degrades pre-migration), migration `20260615300000_competency_training` (competency_skills + employee_competencies + training_records). PDF/Excel matrix export.
- ✅ **Phase 12 — Attendance & GPS** (`/hr/attendance`): daily check-in/out with browser geolocation (Google-Maps link per record), date filter, KPIs (records, on-site, completed, total hours), check-in (select employee → GPS) + per-row check-out (auto hours). New `attendanceService` (audit-logged, degrades pre-migration), migration `20260615320000_attendance`. PDF/Excel.
- ✅ **Phase 13 — Employee Documents Center** (`/hr/documents`): per-employee document completeness (key statutory IDs present/expiring/missing) + cert + upload counts, KPIs (complete/incomplete files, uploads), and an expandable per-employee unified register (statutory docs + certifications + DMS uploads from `documents`). PDF/Excel.
- ✅ **Phase 14 — HR Hub + Audit & Export Polish** (`/hr/hub`): hub with a live attention strip (docs expired/≤30d, certs expiring ≤60d, leave + timesheet pending) + module grid; audit-logged new write paths (competency, attendance); PDF/Excel verified across all pages. See `HR-ENTERPRISE-COMPLETE.md`.

**🎉 All 14 modules complete.** Full report: [`HR-ENTERPRISE-COMPLETE.md`](./HR-ENTERPRISE-COMPLETE.md). Migrations to apply: `20260615300000_competency_training`, `20260615320000_attendance`.
